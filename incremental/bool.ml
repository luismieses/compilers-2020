(* bool.ml - Adding Booleans to the compiler *)
open Printf
open Int64

type binop = Plus | Minus | Mult
             | And | Or | Less | LessEq | Greater | GreaterEq | Eq | Ne
...
let rec compile (e : expr) (env : env) : instruction list =
  match e with
  | Num n -> if n > max_cobra_int || n < min_cobra_int then
       failwith ("Integer overflow: " ^ (Int64.to_string n))
     else
       [ IMov(Reg(RAX), Const(Int64.shift_left n 1)) ]
  | Bool true -> [IMov (Reg(RAX), Const(const_true))] (* valor cierto *)
  | Bool false -> [IMov (Reg(RAX), Const(const_false))] (* valor falso *)
